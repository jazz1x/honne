...
각 턴: 지정된 인격의 시스템 프롬프트를 정신적으로 적용 → 2~3문장 생성 → 레이블을 앞에 붙여 출력.

| # | 인격 | 입장                       | 레이블                       |
|---|------|--------------------------|------------------------------|
| 1 | antipattern | TOPIC에 대한 공격          | `**[비효율 — {name}]**`     |
| 2 | signature   | 1턴에 대한 반박            | `**[강점 — {name}]**`       |
| 3 | antipattern | 2턴에 대한 재반박           | `**[비효율 — {name}]**`     |
| 4 | signature   | 최종 주장                  | `**[강점 — {name}]**`       |

## 6단계: 심판자의 판결

`.honne/personas/judge.md` 시스템 프롬프트를 적용합니다. 2~3문장의 판결을 생성합니다: 어떤 접근이 상황적으로 더 적절하며, 그 이유는 무엇인지.
