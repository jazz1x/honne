---
name: crush
version: 0.0.5
description: >
  あなたのペルソナの二つの声(アンチパターン対シグネチャ)がトピックについて交わすライブディベート。
  Triggers: "crush", "debate personas", "personas fight", "/honne:crush".
ssl:
  scheduling:
    anti_triggers:
      - ".honne/personas/{antipattern,signature,judge}.md がすべて不在の場合 (persona を先に実行)"
  structural:
    scenes:
      - "Step 1: トピック取得"
      - "Step 2: ペルソナ検証"
      - "Step 3: ペルソナロード"
      - "Step 4: ラウンド1 — アンチパターン攻撃"
      - "Step 5: ラウンド1+ラウンド2 — シグネチャ & カウンター"
      - "Step 6: 審判者の判決"
    branches:
      - "Step 1: skill_args が空 → プレーンテキスト質問"
      - "Step 2: 3つすべて exit 0 → 進行"
      - "Step 2: 3つすべて exit 66 → persona 案内後に停止"
      - "Step 2: 混在 → whoami+persona 案内後に停止"
    resumable: false
  logical:
    tools: ["bash", "Read"]
    side_effects:
      reads:
        - ".honne/personas/*.md"
      writes: []
      deletes: []
      network: []
    idempotent: true
    rollback: null
---

# honne — ペルソナ討論（ライブ）

**呼び出されたら、ステップ1からステップ6まで順番に実行してください。スキル引数が提供されていれば、それを討論トピックとして使用し、そうでなければ尋ねてください。**

## ステップ1: トピック取得

`skill_args`が空でなければ: `TOPIC = skill_argsをスペースで結合`

それ以外: プレーンテキスト質問: "二つのペルソナが討論するトピックは?"

返答から`TOPIC`を設定します。

## ステップ2: ペルソナ検証

存在するペルソナファイル数をカウント:

```bash
bash "${CLAUDE_PLUGIN_ROOT}/scripts/honne" persona check --persona .honne/personas/antipattern.md
```
```bash
bash "${CLAUDE_PLUGIN_ROOT}/scripts/honne" persona check --persona .honne/personas/signature.md
```
```bash
bash "${CLAUDE_PLUGIN_ROOT}/scripts/honne" persona check --persona .honne/personas/judge.md
```

各 exit 0（存在）または exit 66（不在）。組み合わせ結果に応じて分岐:

- **3つとも exit 0** → ステップ3へ進む。
- **3つとも exit 66** → ユーザーへ通知: "ペルソナがありません。まず `/honne:persona` を実行して生成してください。" 停止。
- **混合** → ユーザーへ通知: "ディベートには antipattern・signature ペルソナと審判者がすべて必要です。現在のペルソナは1軸のみ検出されました。セッションを追加収集し `/honne:whoami` + `/honne:persona` を再実行してください。" 停止。

## ステップ3: ペルソナロード

メンタルコンテキストに3つのファイルをすべて読み込みます:
- `.honne/personas/antipattern.md` → システムプロンプト と`# `ヘッダーから`name`を抽出
- `.honne/personas/signature.md` → システムプロンプト と`name`を抽出
- `.honne/personas/judge.md` → システムプロンプト を抽出

ヘッダーの後の最初の`# Name`行を読んで、各`name`を抽出します。

## ステップ4–5: 討論 (4ターン)

各ターン: 指定されたペルソナのシステムプロンプトをメンタルに適用 → 2~3文を生成 → ラベルを前置して出力。

| # | ペルソナ | 立場                     | ラベル                        |
|---|---------|-------------------------|-------------------------------|
| 1 | antipattern | TOPIC への攻撃           | `**[アンチパターン — {name}]**` |
| 2 | signature   | 1ターンへの反論           | `**[シグネチャ — {name}]**`    |
| 3 | antipattern | 2ターンへの再反論         | `**[アンチパターン — {name}]**` |
| 4 | signature   | 最終主張                 | `**[シグネチャ — {name}]**`    |

## ステップ6: 審判者の判決

`.honne/personas/judge.md`システムプロンプトを適用します。2~3文の判決を生成します: どのアプローチがより状況的に適切で、その理由は何か。

ラベル: `**[判決]**` その後判決。

---

## 出力形式

この構造でフルトランスクリプトを出力します:

```markdown
**トピック**: {TOPIC}

**[アンチパターン — {name}]**
{ラウンド1攻撃}

**[シグネチャ — {name}]**
{ラウンド1反論}

**[アンチパターン — {name}]**
{ラウンド2再反論}

**[シグネチャ — {name}]**
{ラウンド2最終}

**[判決]**
{審判者判決}
```

ファイル書き込みなし。トランスクリプトは一時的 — 現在のセッションのみで表示されます。
