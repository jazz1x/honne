...
すべての処理はローカルで完結します。ネットワーク呼び出しなし、解析トラッキングなし。すべての主張は persona ファイルに入る前に HITL 承認を経ます。データは `.honne/` 配下に平文の JSONL として — 持ち運びやすく、見やすく、削除しやすい形で保持されます。
...
| スキル | コマンド | 役割 |
|--------|----------|------|
| **whoami** | `/honne:whoami` | メインオーケストレーター。6 軸ペルソナ + 軸ごとの HITL 承認。 |
...
python3 がない、または 3.9 未満の場合、スクリプトは exit code 4 で終了します。`jq`、`ripgrep` は不要になりました。
...
`honne` がリストに表示されていれば OK。以下の 3 つの本体スラッシュコマンドが補完されれば準備完了:
...
step 1 > スキャン範囲 — repo (現在のプロジェクト) / global (全プロジェクト)?
user   > global

step 2 > transcripts をインデックス中 ~/.claude/projects/… (127 ファイル)
step 3 > 機密パターンをマスク (12 パターン) → キャッシュ .honne/cache/scan.json
step 4 > 6 軸を抽出: lexicon, cadence, stance, ritual, antipattern, evolution
step 5 > 軸ごとに HITL — [y]es / [n]o / [e]dit

        軸 1 · lexicon   : 「一旦」「〜してみます」、英↔日のコードスイッチ頻繁  → [y/n/e]?
user   > y

        ... (軸 2–6 も同様) ...

step 6 > ペルソナスナップショット + 経時資産を記録しますか?
user   > y

✓ 保存: .honne/persona.json
✓ 保存: docs/honne.md
✓ append: .honne/assets/claim.jsonl (6 件)
```

2 回目以降の実行後は、過去のプロファイルと比較可能:
...
- 機密パターン (API キー, トークン, webhook, メール, 電話番号, ホームパス, IP, クレジットカード — 全 12 パターン) は quote 保存前にマスク処理されます。`scripts/honne_py/redact.py` を参照。
...
- [honne](https://github.com/jazz1x/honne) — 証拠ベースの自己観察 (6 軸ペルソナ)
