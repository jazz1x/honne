...
user scope を推奨します — 全プロジェクトを横断するグローバルな transcripts スキャンが有効になります。local scope (プロジェクト単位) も対応していますが、自己観察の範囲が単一プロジェクトに限定されます。

## スキル

| スキル | コマンド | Alias | 役割 |
|--------|----------|-------|------|
| **honne** | `/honne:honne` | `/honne:whoami` · `/honne:hon-me` | メインオーケストレーター。6 軸ペルソナ + 軸ごとの HITL 承認。 |
| **lexi** | `/honne:lexi` | — | 語彙 (Lexicon) 軸のみ (高頻度語彙、コードスイッチング比率、擬音語・擬態語)。 |
| **compare** | `/honne:compare` | `/honne:diff` · `/honne:hon-back` | 読み取り専用の振り返り。蓄積された資産を読み、経時的な変化を提示。transcript 再スキャン / LLM 再分析なし。 |

Alias は 2 系統: **セマンティック** (`whoami`, `diff`) — Unix に馴染みのあるエントリ。**ブランドプレフィックス** (`hon-me`, `hon-back`) — harnish `har-*` 方式で marketplace 上の honne ファミリーを視覚的にグループ化。どちらも同じ本体スキルへ redirect。
...
- **whoami** / **hon-me** — `honne` の alias。`whoami` は Unix に馴染み、`hon-me` は harnish `har-*` ブランドプレフィックス規約に対応。
- **diff** / **hon-back** — `compare` の alias。`diff` は汎用語、`hon-back` は「honne、振り返る」と読める。
...
- **honne** — transcripts からの自己観察 (*知る*)
- hanashi (計画中) — 伝達 / コミュニケーション (*伝える*)
