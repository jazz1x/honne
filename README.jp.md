...
| スキル | コマンド | 役割 |
|--------|----------|------|
| **whoami** | `/honne:whoami` | メインオーケストレーター。6 軸ペルソナ + 自律証拠収集 + LLM ナラティブ合成。 |
...
```
 トランスクリプト (~/.claude/projects/**/*.jsonl)
      │
      │  honne  ──→  .honne/persona.json + docs/honne.md  (6 軸スナップショット)
      │                  │
...
step 3 > 6 軸を自律抽出 [lexicon, reaction, workflow, obsession, ritual, antipattern]
...
→ transcripts をスキャン → 6 軸を自律抽出 → 主張を記録
→ LLM が軸ごとの解説 + 一行評を合成
...
→ lexi が transcripts をスキャン → lexicon 軸のみ → 自律記録
→ lexicon 軸の claim 資産を記録
```
