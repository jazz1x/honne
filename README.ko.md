...
```
 트랜스크립트 (~/.claude/projects/**/*.jsonl)
      │
      │  honne  ──→  .honne/persona.json + docs/honne.md  (7축 스냅샷)
      │                  │
...
python3 가 없거나 3.9 미만이면 스크립트는 exit code 71 로 종료됩니다.

## 설치

### 옵션 1 — `npx skills add` (권장)

Claude Code, Cursor, Codex, Windsurf 등 skills.sh 호환 에이전트에서 동작합니다.

```bash
npx skills add jazz1x/honne                  # ./.claude/skills/ 에 설치 (프로젝트)
npx skills add jazz1x/honne -g               # ~/.claude/skills/ 에 설치 (글로벌, honne 권장)
npx skills add jazz1x/honne --list           # 설치 전 스킬 목록 확인
npx skills add jazz1x/honne --skill whoami   # 단일 스킬만 설치
```
...
글로벌 설치(`-g`) 를 권장합니다 — honne 는 모든 프로젝트의 transcripts 를 읽으므로, 프로젝트를 가로지르는 이력이 있을수록 자기 관찰이 풍부해집니다.

### 옵션 2 — Claude Code 네이티브 플러그인
...
`honne` 이 리스트에 보여야 합니다. 아래 다섯 슬래시 명령이 자동완성되면 준비 완료:
...
| `.honne/persona.json` | 현재 7축 프로필 스냅샷 |
...
- 민감 패턴 (API 키, 토큰, webhook, 이메일, 전화번호, 홈 경로, IP, 신용카드 — 총 18 패턴) 은 quote 저장 전 가림 처리됩니다. `scripts/honne_py/redact.py` 참조.
...
- [honne](https://github.com/jazz1x/honne) — 증거 기반 자기 관찰 (7축 페르소나)
